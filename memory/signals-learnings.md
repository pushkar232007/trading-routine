...
- **The ratchet paid off again — MRK booked +17.41% instead of round-tripping (validated 2026-08-28, 2nd instance).** Same mechanics as the META 7/17 win: MRK ran to +18-21%, was tightened 10%→7% on 8/19 per the 15-30% band, and on 8/28 the tape pulled back at the open enough to trip the 7% trail (filled 09:33 ET @ $147.82) → **realized +$504.16 / +17.41%** rather than bleeding the unrealized run back toward breakeven. A loose 10% trail would NOT have fired here (the pullback was <10% off the ~$156.9 hwm). Two-for-two confirmation: keep tightening winners on schedule and NEVER loosen — the tightened stop is what converts an extended runner into a booked gain when a defensive name (not just a tech name) rolls over off a high. Corollary: an at-the-open stop fill is discovered by the midday check as an already-executed close — treat it as a booked trade + Telegram, not a discretionary decision.

- **An overnight oil/macro risk-off read is often an intraday head-fake — don't over-manage the book on it (2026-07-22).** The 7/22 pre-market read was firmly RISK-OFF: Brent re-spiked to ~$94 (Iran ceasefire stalled), chips cooling into GOOGL/TSLA earnings, futures red, and I flagged FDX's oil flip-flop risk as "materializing." Yet the tape rallied all day and **all 3 closed GREEN** — NVDA recovered from -2.83% at the open to +0.18% (day +2.33%), and oil-exposed FDX actually rose +1.85%. This echoes the 7/16→7/17 oil head-fakes. Lesson: a scary overnight macro read (oil, geopolitics) is a reason to WATCH, not to pre-emptively cut/hedge a stop-protected holding — the guardrails (10% trail, -7% cut) already cap the downside if the risk is real, and reacting to the pre-market alarm instead of live intraday price repeatedly would have sold winners into a reversal. Hold the plan; let the stops do the risk management. (Pairs with the 7/15 "evaluate the LIVE gate, not the pre-market's pre-open guess" note.)
...
- **A pre-flagged, DATED bear catalyst is not a head-fake — the -7% cut executed on schedule (2026-07-27).** Memory had named "Kimi K3 weights drop 7/27" for weeks as NVDA's specific DeepSeek-style compute-scarcity risk. On 7/27 the weights actually dropped, NVDA sold ~-5.3% intraday and hit -7.5% from entry → the hard -7% loss-cut fired at midday (sold 14 sh @ $195.93, realized -$221.20 / -7.46%). No pre-logged thesis reason to override → clean rules-based exit. Lesson + the key contrast with the 7/22 head-fake note: an *overnight geopolitical/oil* risk-off read is fluid and often reverses intraday (WATCH, don't over-manage); a *scheduled, structural* event on a known date (a free frontier model's weights releasing, an earnings print) is a different animal — when it lands as feared, respect the rule and let the cut work rather than inventing a hold reason. Distinguish head-fake (fluid headline, no fixed date) from a genuine dated binary materializing. Mechanics reminder: shares held by a live trailing stop show qty_available 0, so `close` 403s first — cancel ONLY that symbol's stop via a targeted DELETE (never `cancel-all`, which drops the other holdings' stops), then close.

- **Don't let the deployable list be a single RATE-SENSITIVE name in a persistent hawkish regime (2026-09-04 weekly review).** CAT has been THE named non-healthcare deployable for a full month, and its macro-timing gate has failed every single routine — not because the thesis weakened (it strengthened: $11.5B credit facilities, record power-gen backlog) but because it's a rate-sensitive industrial cyclical and the post-Warsh regime kept pricing Sept hikes (culminating in the HOT August NFP 9/4 that vindicated the deferral). The gate was individually correct each time, yet the aggregate is 5 straight weeks at 0/3 slots and a book that only beats the S&P when the S&P falls. Lesson: a deploy gate keyed to a macro variable that isn't going to turn on your schedule = structural under-deployment dressed up as discipline. When the ONLY named deployable is rate-sensitive and the regime is durably hawkish, either accept it won't clear until the regime turns AND name a non-rate-sensitive alternative diversifier, or you'll keep logging "defer, re-vet later" indefinitely. Deferral is right for a dated binary (the NFP); it's a trap when the blocking condition is a standing regime.
...
- **A 1-name / >95%-cash book is a RISK STATE, not a neutral default — escalate the deployment-floor when it happens (2026-09-11, weekly review).** Wk 9/7 exited with the book thinned to a SINGLE holding (CI) + ~97% cash after two trailing-stop-outs in three days (GEHC 9/8 −5.00%, FDX 9/10 −1.90%) — the most under-deployed state in the account's history, and the 6th straight week at 0/3 slots. Both exits were correct (price-based stops working, disciplined non-pre-emption per 7/22); the miss was having NO vetted replacement queued. The outperformance (+0.51 pts) was purely defensive (cash in a down S&P week) — the "we only beat the index when it falls" one-directional book in its most acute form. **Rule:** when the book thins to a single position OR drops <~5% invested, the deployment-floor decision ESCALATES from "name a candidate OR log a reason" to "the next pre-market MUST surface a vetted, tradeable candidate (concrete entry + earnings date + live-sampled clean spread)." Staying ~97% cash is acceptable ONLY while an active broad rout (falling-knife rule) is in force, and must be RE-justified each routine, not carried silently. Corollary: don't wait on a single perpetually-gated name (CAT, rate-sensitive in a hawkish regime) — always carry a second, non-rate-sensitive horse (vetted consumer-staples name) so deployment isn't hostage to one sector's macro gate.
